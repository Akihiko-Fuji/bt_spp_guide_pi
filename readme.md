...
> ⚠️ **重要な前提**
> - 本手順を適用しても **必ず 100% 成功するとは限りません**。SPP 対応機器ごとに挙動が微妙に異なり、接続完了までの待ち時間や再接続タイミングが一致しないケースがあります。
> - SPP 端末側がスリープ・省電力状態に入った後、復帰直後に自動で再接続できない個体が確認されています。再起動直後の初回接続は高い確率で成功するものの、再接続は機器依存です。
> - BlueZ のシリアルプロトコルを有効化した構成は、再接続コードを組み込んでも不安定なケースが多く、検証では **Blueman を併用した接続管理のほうが再現性が高い** という結論に至っています。
> - 以上を踏まえ、あくまで「高い成功率を狙うためのベースライン」として活用し、運用環境の機器特性に応じて調整してください。
...
> ℹ️ **BlueZ 設定例は参考用**
> - 本ガイドの後半では BlueZ ベースの設定例を記載していますが、筆者の検証ではシリアルプロトコルを有効化した BlueZ は再接続を含めた長時間運用で不安定になるケースが多く、実運用では Blueman 経由の接続管理を主軸としています。
> - BlueZ の手順は仕組み理解や切り分けの参考として位置づけ、安定運用を重視する場合は Blueman を利用した接続を優先してください。

blueman導入後にはX window system上のタブにマネージャーのアイコンが追加されます。機能が重複しますので古いBluetoothの管理アイコンは消しておくのが適切です。
